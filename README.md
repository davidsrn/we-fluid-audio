...
- Frequency → color mapping: each band maps to a configurable hue range, with per-band color pickers in WE settings
- Energy smoothing: exponential moving average to cut down on jittery splat bursts
...
- Band layers: bass paints to a back layer, mids to the middle, treble to the front, so bass blobs don't overwrite treble detail
- Bloom: configurable intensity and threshold
- All settings are individually toggleable from the Wallpaper Engine settings panel
