...
function audioSplats (amount, radius, colorMult) {
    for (let i = 0; i < amount; i++) {
        const color = config.COLORFUL ? generateColor() : Object.assign({}, config.POINTER_COLOR.getRandom());
        color.r *= colorMult;
        color.g *= colorMult;
        ...
        const dx = 1000 * (Math.random() - 0.5);
        const dy = 1000 * (Math.random() - 0.5);
        splatWithRadius(x, y, dx, dy, color, radius);
    }
}

function generateBandColor (hueMin, hueMax) {
    if (!config.COLORFUL) {
        return Object.assign({}, config.POINTER_COLOR.getRandom());
    }
...
function audioSplatsPositioned (amount, radius, colorMult, xBias, xSpread, hueMin, hueMax, layerIndex = 0) {
    for (let i = 0; i < amount; i++) {
        const color = generateBandColor(hueMin, hueMax);
        color.r *= colorMult;
        color.g *= colorMult;
        color.b *= colorMult;
